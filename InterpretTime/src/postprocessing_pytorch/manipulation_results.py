...
class ScoreComputation:
    ...
    def __init__(
        ...
        assert noise_type in ["normal_distribution","zeros","global_mean","local_mean","global_gaussian",
                              "local_gaussian"]
    ...
    def _return_baseline(
        ...
        if type_baseline == "zeros":
            ...
            if self.model_output=="probabilities":
                ...
                baseline = np.random.normal(loc=0,scale=1, size=s[:1].shape)
            expected_value = (
                fwd_function(baseline)
            )
    ...
    def __compute_score_pred(
        ...
        correct_classification = to_numpy(argmax(score2)) == np.argmax(self.target_encoded, axis=1)
        original_pred_score = to_numpy(score2)[self.pred_idx.astype(bool) ]
        ...
        correct_classification = to_numpy(argmax(score3)) == np.argmax(self.target_encoded, axis=1)
    ...
    def __init_samples(self,encoder):
        ...
        if self.np_signal.shape[0] != len(list_sample_names):
        ...
        if self.model_output=="probabilities":
            ...
            # variables and functions for classification, aeon/sktime/scikit-learn models
            # defining data structures
        ...
        for sample_signal in batched_sample:
            with torch.no_grad():
                score_pred = cat_function(
                    score_pred,
                    fwd_function(sample_signal)
                )

        # get clean signals
        self.baseline = self.np_signal.copy()
        self.target_encoded = np.array(target_encoded_all)
        pred_idx = zeros_like(score_pred)
        idx_max =  argmax(score_pred)
        pred_idx[torch.arange(score_pred.shape[0]), idx_max] = 1  # predicted index
        self.score_pred =  to_numpy(score_pred)  # predicted score
        self.pred_idx = to_numpy(pred_idx) # predicted index
